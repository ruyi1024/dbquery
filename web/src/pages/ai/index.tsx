...
const Index: React.FC = () => {
  ...
  const asyncFetch = (values: {}) => {
    console.info(values);
    setLoading(true);
    const params = { ...values, "query_type": "ai" };
    const headers = new Headers();
    headers.append('Content-Type', 'application/json');
    fetch('/api/v1/ollama/ai_run', {
      method: 'post',
      headers: headers,
      body: JSON.stringify(params),
    })
        ...
        <Col span={20}>
          <Card>
            ...
            >
              ...
              <Form.Item wrapperCol={{ offset: 0, span: 16 }}>
                <Space>
                  <Button type="primary" htmlType="submit" icon={<RightSquareOutlined />}>AI优化提交</Button>
                </Space>
              </Form.Item>
            </Form>
          </Card>
          ...
          <Card>
            ...
            {tableDataSuccess == true && tableDataMsg != "" &&
              <Alert type="success" message={"DeepSeek服务调用成功"} banner />

            }

            <ReactMarkdown>{tableDataMsg}</ReactMarkdown>
          </Card>
